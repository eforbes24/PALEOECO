...
class EvolSearch:
    def __init__(self, evol_params, continuous_initial_pop):
        """
        ...
        rand_genotype_fitness = self.fitness_function(rand_continuous_genotype)
        assert (
        ...
        if self.optional_args:
            if len(self.optional_args) == 1:
                return self.fitness_function(
                    self.continuous_pop[individual_index, :], self.optional_args[0]
                )
            else:
                return self.fitness_function(
                    self.continuous_pop[individual_index, :], self.optional_args[individual_index]
                )
        else:
            return self.fitness_function(self.continuous_pop[individual_index, :])
    ...
    def get_best_individual(self):
        ...
        return self.continuous_pop[best_individual_index, :]
